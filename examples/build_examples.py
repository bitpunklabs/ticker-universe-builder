...
def measurement(market: str) -> dict:
    ...
    block = {
        "liquidity": {
            "basis": "measured",
            "method": "cross-sectional percentile of mean daily turnover",
            "window": "30d",
            "source": source,
        },
        "quality": {
        ...
        "heat": {
            "basis": "judged",
            "method": "turnover jump confirmed against exchange data, never a single-day move",
        },
        "beta_strength": {
            "basis": "measured",
            "method": f"absolute OLS beta against {FACTOR_LABEL[market]}, beta 2.0 reads 100",
            "window": "180d",
            "source": source,
        },
        "beta_stability": {
            "basis": "measured",
            "method": "agreement of the beta estimate across the two halves of the window",
            "window": "180d",
            "source": source,
        },
    }
    span = {
        "basis": "measured",
        "method": f"OLS of daily returns on {FACTOR_LABEL[market]}",
        "window": "180d",
        "source": source,
    }
    if market == "crypto":
        block["factor_r2"] = span
        block["independence"] = {**span, "method": "derived as 100 - factor_r2 by the builder"}
    else:
        block["independence"] = {
            **span, "method": f"100 - the OLS R-squared of daily returns on {FACTOR_LABEL[market]}",
        }
    return block
...
PREAMBLE = """> Generated by `python examples/build_examples.py` from `examples/seeds/{market}.tsv`.
...
def render(market: str, snapshot: dict, spec: dict, folder: Path) -> dict:
    ...
    (folder / "universe.md").write_text(
        PREAMBLE.format(market=market) + render_markdown(universe, report), encoding="utf-8"
    )
    ...
    if changes_path.is_file():
        ...
        (folder / "maintenance.md").write_text(
            PREAMBLE.format(market=market) + render_markdown(reviewed, review_report),
            encoding="utf-8",
        )
    return universe
